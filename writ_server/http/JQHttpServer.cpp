...
#define JQHTTPSERVER_SESSION_PROTECTION( functionName, ... ) \
    ...
    { \
        qDebug()<<this_<<contentLength_<<waitWrittenByteCount_;\
...
static QString replyOptionsFormat(
        ...
        "Access-Control-Allow-Headers: Content-Type,auth,pageSize,currentPage,signalType,Authorization\r\n"
        "\r\n"
    );
...
//        RTALOG_INFO(TaskLog::SYSTEM,"JQHttpServer::Session::replyRedirects: already reply");
        return;
    }
...
void JQHttpServer::Session::replyJsonObject(const QJsonObject &jsonObject, const int &httpStatusCode)
{
    ...
    if ( alreadyReply_ )
    {
        ...
        //RTALOG_INFO(TaskLog::SYSTEM,"JQHttpServer::Session::replyJsonObject: already reply");
        return;
    }
...
void JQHttpServer::Session::replyJsonArray(const QJsonArray &jsonArray, const int &httpStatusCode)
{
    ...
    if ( alreadyReply_ )
    {
        qDebug() << "JQHttpServer::Session::replyJsonArray: already reply"<<QThread::currentThreadId();
        return;
    }
    ...
    alreadyReply_ = true;
    //RTALOG_INFO(TaskLog::SYSTEM,"JQHttpServer::Session::replyJsonObject: alreadyReply_ = true");
    if ( ioDevice_.isNull() )
    {
        qDebug() << "JQHttpServer::Session::replyJsonArray: error1";
      //  RTALOG_INFO(TaskLog::SYSTEM,"JQHttpServer::Session::replyJsonObject: error1");
        this->deleteLater();
        return;
    }
...
void JQHttpServer::Session::replyFile(const QString &filePath, const int &httpStatusCode)
{
    ...
    if ( alreadyReply_ )
    {
        qDebug() << "JQHttpServer::Session::replyFile: already reply"<< QThread::currentThreadId();
        return;
    }
    ...
    //RTALOG_INFO(TaskLog::SYSTEM,"JQHttpServer::Session::replyFile: alreadyReply_ = true");
    if ( ioDevice_.isNull() )
    {
...
void JQHttpServer::Session::replyImage(const QString &imageFilePath, const int &httpStatusCode)
{
    ...
    if ( alreadyReply_ )
    {
        qDebug() << "JQHttpServer::Session::replyImage: already reply"<<QThread::currentThreadId();
        return;
    }
...
void JQHttpServer::Session::replyBytes(const QByteArray &bytes, const int &httpStatusCode)
{
    ...
    if (alreadyReply_)
    {
        qDebug() << "JQHttpServer::Session::replyBytes: already reply"<<QThread::currentThreadId();
...
void JQHttpServer::Session::replyOptions()
{
    ...
    if ( alreadyReply_ )
    {
        qDebug() << "JQHttpServer::Session::replyOptions: already reply"<<QThread::currentThreadId();
        return;
    }
...
//                qDebug() << buffer_;
                ...
                if ( ( requestMethod_.toUpper() == "GET" ) ||
                     ...
                     ( ( requestMethod_.toUpper() == "PATCH" ) && ( ( contentLength_ > 0 ) ? ( !buffer_.isEmpty() ) : ( true ) ) ) ||
